...
monthly_employee_metrics as (
    ...
    select date_month,
        ...
        sum(case when date_month = {{ dbt.date_trunc('month', wh_end_employment_date) }} then 1 else 0 end) as churned_workers_this_month
...
monthly_active_employee_metrics as (
    ...
    select date_month,
        count(distinct employee_id) as active_employees_this_month,
        ...
        sum(case when date_month = {{ dbt.date_trunc('month', wph_end_employment_date) }} 
                then 1 else 0 end) as churned_employees_this_month,
        sum(case when date_month >= {{ dbt.date_trunc('month', effective_date) }}
                and (date_month <= {{ dbt.date_trunc('month', wh_end_employment_date) }}  
                or wh_end_employment_date is null) 
                then 1 else 0 end) as active_workers_this_month
    from end_of_month_history
...
monthly_active_worker_metrics as (
    ...
    select date_month,
        count(distinct worker_id) as active_workers_this_month
    where (date_month >= {{ dbt.date_trunc('month', effective_date) }}
        and date_month <= {{ dbt.date_trunc('month', wh_end_employment_date) }})
            or wh_end_employment_date is null
    group by 1
...
monthly_summary as (
    ...
    select date_month,
        ...
        monthly_active_employee_metrics.*,
        monthly_active_worker_metrics.*
